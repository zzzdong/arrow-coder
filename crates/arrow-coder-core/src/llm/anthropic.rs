...
impl BackendLike for AnthropicBackend {
    async fn complete(
        ...
        if let Ok(req_json) = serde_json::to_string(&request) {
            tracing::info!(target: "llm.anthropic.request", body = %req_json, "Anthropic API request");
        }
        ...
        // Log response body (full)
        if let Ok(resp_json) = serde_json::to_string(&anthropic_response) {
            tracing::info!(target: "llm.anthropic.response", body = %resp_json, "Anthropic API response");
        }
    ...
    async fn complete_streaming(
        ...
        // Log request body (full)
        if let Ok(req_json) = serde_json::to_string(&request) {
            tracing::info!(target: "llm.anthropic.request", body = %req_json, "Anthropic API streaming request");
        }
