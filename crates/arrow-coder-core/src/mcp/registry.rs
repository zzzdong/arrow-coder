...
impl Tool for McpToolWrapper {
    fn name(&self) -> &'static str {
        // MCP tool names are dynamic, so we must leak a heap string to satisfy
        // the `&'static str` trait bound. We prefix with the server name to keep
        // names unique across servers (the model sees `<server>__<tool>`).
        // NOTE: this leaks one string per wrapper — acceptable while the registry
        // is built once at startup. See refactor-log-s4 D14 for the long-term
        // option of switching `Tool::name` to `String`.
        let qualified = format!("{}__{}", self.info.server_name, self.info.tool.name);
    ...
    fn description(&self) -> &'static str {
        ...
        let desc = format!("[MCP:{}] {}", self.info.server_name, base);
...
impl McpRegistry {
    /// Discover tools from all (non-disabled) servers and wrap each as an
    /// [`arrow_coder_core::tools::base::Tool`] so they can be handed to the
    /// agent loop alongside the built-in tools.
    ///
    /// Tool names are qualified as `<server>__<tool>` to avoid cross-server
    /// collisions. Discovery failures are logged and skipped (one bad server
    /// must not take down the whole agent).
    ///
    /// The returned wrappers share `registry` (same `Arc`), so tool calls reuse
    /// the already-established transports and cache.
    pub async fn discover_tool_wrappers(
        registry: Arc<McpRegistry>,
        servers: &[McpServerConfig],
    ) -> Result<Vec<Arc<dyn crate::tools::base::Tool>>> {
        let infos = registry.get_tools(servers).await?;
        let mut wrappers: Vec<Arc<dyn crate::tools::base::Tool>> = Vec::new();
        for info in infos.values() {
            // Skip tools explicitly disabled per-server.
            if let Some(ref disabled) = info.server_config.disabled_tools {
                if disabled.contains(&info.tool.name) {
                    continue;
                }
            }
            wrappers.push(Arc::new(McpToolWrapper::new(info.clone(), registry.clone())));
        }
        Ok(wrappers)
    }
}
