...
impl ToolPrompt {
    ...
    pub fn get(tool_name: &str) -> Option<&'static str> {
        match tool_name {
            ...
            "str_replace_editor" => Some(include_str!("tool_prompts/str_replace_editor.md")),
    ...
    pub fn all() -> HashMap<&'static str, &'static str> {
        ...
        map.insert("str_replace_editor", include_str!("tool_prompts/str_replace_editor.md"));
...
pub fn format_system_prompt(
    ...
    // Append the shared output-formatting guidelines (code-fence rules, etc.)
    // so every agent profile emits Markdown that strict CommonMark parsers can
    // render without mis-closing code blocks.
    content.push_str("\n\n");
    content.push_str(FORMATTING_GUIDELINES);
    content
}

/// Shared output-formatting guidelines appended to every agent's system
/// prompt. They teach the model to emit strict-CommonMark-compatible Markdown
/// (especially well-formed code fences), which keeps the webview renderer from
/// mis-closing code blocks when the model appends a language name to a closing
/// fence or truncates mid-block.
pub const FORMATTING_GUIDELINES: &str = include_str!("formatting_guidelines.md");
