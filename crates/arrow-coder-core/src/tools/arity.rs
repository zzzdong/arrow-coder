...
lazy_static! {
    ...
    pub static ref ARITY: HashMap<&'static str, usize> = {
        ...
        for cmd in &[
            "bazel", "brew", "bun", "cdk", "cf", "cmake", "composer",
            "consul", "crictl", "deno", "docker", "eksctl", "firebase", "flyctl",
            "gh", "go", "gradle", "helm", "heroku", "hugo", "ip",
        ...
        // Common VCS/build tools where only the top-level command should be
        // part of the permission pattern (arity 1).
        ...
        for cmd in &[
            ...
            "consul kv",
            "deno task",
            "docker builder", "docker container",
...
mod tests {
    ...
    #[test]
    fn test_get_arity() {
        assert_eq!(get_arity("git"), Some(1));
        assert_eq!(get_arity("cargo"), Some(1));
        assert_eq!(get_arity("cargo run"), Some(2));
        assert_eq!(get_arity("docker compose"), Some(2));
