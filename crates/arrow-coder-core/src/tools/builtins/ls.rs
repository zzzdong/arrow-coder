...
impl Tool for LsTool {
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "The path to the directory to list. If empty or not provided, lists the current working directory."
                },
                "depth": {
                    "type": "integer",
    ...
    /// Model sees a bounded projection of directory entries; the full listing
    /// stays in the session log as the canonical `value`.
...
mod tests {
    ...
    fn test_ls_default_path() {
        let args: LsArgs = serde_json::from_value(serde_json::json!({"depth": 0})).unwrap();
