...
impl TodoState {
    ...
    /// Snapshot the list as plain JSON values (for `SessionEvent::TodoWrite`).
    pub fn snapshot(&self) -> Vec<serde_json::Value> {
        self.items.iter().map(|i| serde_json::to_value(i).unwrap_or_default()).collect()
    }

    /// Replace the whole list from a snapshot (used for session replay).
    pub fn restore(&mut self, items: Vec<serde_json::Value>) {
        self.items = items
            .into_iter()
    ...
    /// Borrow the current items (for projection / UI).
    pub fn items(&self) -> &[TodoItem] {
        &self.items
    }

    /// Change a single item's status (manual cancel / trigger from the UI).
